...
function ManageOnClickRadioBtn(){
...
//    console.log(myFlowChartList[Object.keys(myFlowChartList)[0]].operators.draft.properties.title);
    ...
    for (var i = 0; i < Object.keys(myFlowChartList).length; i++) {
        //Création de 3 tr
        myTable += "<tr class='toto' id='" + Object.keys(myFlowChartList)[i] + "'>";
...
//        jQuery('#draft').append(setTitle+' ');
    ...
    function DuplicateLine(idTr) {
        var clonage = jQuery('tr#' + idTr).clone();
        jQuery('#dataTable tbody').append(clonage);
//        for(var i = 0; i < jQuery('tr.toto').size(); i++){
        var count = jQuery('tr.toto').size() + 1;
        var id_NvFl = 'fc' + count;
        jQuery(clonage).attr('id', id_NvFl); //change l'id du 'tr' dupliqué id_+1

        jQuery(clonage).find('input:radio').attr('id_flowchart', id_NvFl); //change l'id de l'input du 'td' du 'tr' dupliqué fc+1

//        myFlowChartList[id_NvFl] = myFlowChartList[Object.keys(myFlowChartList)[idTr]];
        myFlowChartList[id_NvFl] = myFlowChartList[idTr];
        ManageOnClickRadioBtn();

//        console.log(jQuery(Object.keys(myFlowChartList)[i]).flowchart('getData'));
//        console.log(Object.values(Object.keys(myFlowChartList)[i]));
//        }
    ...
    function DeleteLine(idTr) {
        jQuery('tr#' + idTr).remove();
    ...
    $(document).contextmenu({
        delegate: ".toto",
        menu: [
            {title: "Filter", cmd: "filter"},
            {title: "Duplicate", cmd: "duplicate"},
        ...
        select: function (event, ui) {
            ...
            switch (ui.cmd) {
                case "filter":
                    ...
                    break;
                case "duplicate":
                    var id_Tr = jQuery(ui.target).parent('tr').attr('id');
                    DuplicateLine(id_Tr);
                    break;
                case "delete":
                    var id_Tr = jQuery(ui.target).parent('tr').attr('id');
